...
def loginValidation():
    ...
    if user!=None:
        ...
        print(user.Password)
        print(vo.Password)
        if user.Password==vo.Password and (vo.UserName==user.UserName or vo.Emaild==user.EmailId):
        # if user.Password==vo.Password and (vo.UserName==user.UserName or vo.Emaild==user.EmailId):
            if user.Status==1:
                ...
                if user.Role==1:
                    ...
                    print('ACTIVE GROUP USER....',activeGroupUser)
                ...
                return render_template('DashBoard.html',obj=user,activeGroupUser=activeGroupUser,lenActiveGroupUser=len(activeGroupUser),adminPost=adminPost,lenActiveUsers=len(activeUsers),activeUsers=activeUsers,ldp=len(groupPost),lugu=len(unApprovedGroupUserList),lug=len(unApprovedGroup),lup=len(UnApprovedPostList),lus=len(unApprovedUserList),dailyPost=groupPost,unApprovedUserList=unApprovedUserList,UnApprovedPostList=UnApprovedPostList,unApprovedGroup=unApprovedGroup,unApprovedGroupUserList=unApprovedGroupUserList)
